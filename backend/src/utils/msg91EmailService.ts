 ...
 * All email functions using MSG91 API instead of Resend
 * Mirror of emailService.ts functionality for easy switching
 */
...
const EMAIL_PROVIDER = process.env.EMAIL_PROVIDER || 'resend'; // 'resend' or 'msg91'

/**
 * Helper to send email via MSG91 API
 * MSG91 uses template-based email sending
 */
async function sendMsg91Email(options: {
  ...
  // Using MSG91's email sending API with template approach
  const payload = {
    to: Array.isArray(options.to) ? options.to : [options.to],
    from: options.from || MSG91_EMAIL_FROM,
    subject: options.subject,
    html: options.html,
    // MSG91 supports attachments via their API
  ...
  const response = await fetch('https://api.msg91.com/api/v5/email/send', {
    method: 'POST',
    headers: {
      'Authorization': `Bearer ${MSG91_API_KEY}`,
 ...
 * Validates if an email address is active/deliverable
 */
export async function validateEmail(email: string) {
  if (!MSG91_API_KEY) {
    logger.warn('MSG91_API_KEY not configured, skipping email validation');
    return { valid: true, deliverable: true };
  }

  try {
    const response = await fetch(`https://api.msg91.com/api/v5/email/validate`, {
      method: 'POST',
      headers: {
        'Authorization': `Bearer ${MSG91_API_KEY}`,
        'Content-Type': 'application/json'
    ...
    if (!response.ok) {
      throw new Error(`MSG91 validation error: ${response.status}`);
    }

    return await response.json();
  } catch (error) {
    logger.error('MSG91 email validation failed', error);
...
/**
 * Template CSS Inliner via MSG91
 * Automatically inline CSS styles within HTML emails
 */
export async function inlineEmailCSS(html: string) {
  if (!MSG91_API_KEY) {
    return html;
  }

  try {
    const response = await fetch('https://api.msg91.com/api/v5/email/css-inliner', {
      method: 'POST',
      headers: {
        'Authorization': `Bearer ${MSG91_API_KEY}`,
        'Content-Type': 'application/json'
    ...
    if (!response.ok) {
      throw new Error(`MSG91 CSS inliner error: ${response.status}`);
    }

    const data = await response.json();
    return data.html || html;
  } catch (error) {
    logger.error('MSG91 CSS inliner failed, returning original HTML', error);
    return html;
  }
}

// ===========================================================================
// MSG91 Email Functions - Mirror of emailService.ts
// Each function maintains same signature for drop-in replacement
// ===========================================================================
...
export const sendTicketEmail = async (
  ...
  try {
      ...
      <div style="font-family: sans-serif; color: #1e293b; max-width: 600px; margin: 0 auto; border: 1px solid #e2e8f0; border-radius: 12px; overflow: hidden;">
        ...
        <div style="padding: 30px;">
          ...
          <div style="display: flex; gap: 15px; margin-bottom: 30px;">
            <a href="${calendarUrl}" style="background-color: #4f46e5; color: white; padding: 12px 20px; text-decoration: none; border-radius: 6px; font-weight: bold; display: inline-block;">
    ...
    logger.info('Ticket email sent via MSG91', { email });
  } catch (err) {
...
export const sendManagerApprovalEmail = async (email: string, userName: string) => {
  ...
  try {
    ...
    logger.info(`Approval email sent via MSG91 to manager: ${email}`);
  } catch (err) {
...
export const sendEventApprovalEmail = async (email: string, userName: string, eventTitle: string) => {
  ...
  try {
    ...
    logger.info(`Event approval email sent via MSG91 to manager: ${email} for ${eventTitle}`);
  } catch (err) {
...
export const sendEventDeclineEmail = async (email: string, userName: string, eventTitle: string, reason: string) => {
  ...
  try {
    ...
    logger.info(`Event decline email sent via MSG91 to manager: ${email} for ${eventTitle}`);
  } catch (err) {
...
export const sendPartnerContractEmail = async (
  email: string,
  partnerName: string,
  partnerType: 'Event Manager' | 'Store Owner'
) => {
  if (!MSG91_API_KEY) return;
  ...
  try {
    const html = `
      <div style="font-family: sans-serif; color: #1e293b; max-width: 600px; margin: 0 auto; border: 1px solid #e2e8f0; border-radius: 12px; overflow: hidden;">
        <div style="background-color: #050505; padding: 40px; text-align: center; color: white;">
          <h1 style="margin: 0; font-weight: 900; text-transform: uppercase; letter-spacing: -0.05em;">City Pulse</h1>
          <p style="margin: 5px 0 0; color: #10B981; font-weight: bold; letter-spacing: 0.1em; text-transform: uppercase; font-size: 12px;">Partner Onboarding</p>
        </div>
        <div style="padding: 40px;">
          <h2>Dear ${partnerName},</h2>
          <p>Welcome to the City Pulse collective. Your <strong>${partnerType}</strong> account has been activated and you're ready to start creating events and managing your presence on the platform.</p>

          <div style="background-color: #f8fafc; border: 1px solid #e2e8f0; border-radius: 8px; padding: 20px; margin: 20px 0;">
            <p style="margin: 0 0 8px; font-size: 12px; font-weight: bold; text-transform: uppercase; color: #64748b;">GETTING STARTED</p>
            <p style="margin: 4px 0;">• Log in to your dashboard to create your first event or store listing</p>
            <p style="margin: 4px 0;">• Complete your profile with contact details and branding</p>
            <p style="margin: 4px 0;">• Explore our analytics tools to track your performance</p>
            <p style="margin: 4px 0;">• Reach out to support anytime for assistance</p>
          </div>

          <p style="color: #475569; line-height: 1.6;">For commission structures, payment schedules, content guidelines, and partnership terms, please refer to the official partner agreement document that will be shared separately via secure document management. If you haven't received it within 24 hours, please reply to this email.</p>
          ...
          <p style="color: #94a3b8; font-size: 13px;">If you have any questions, please reply to this email and our team will assist you.</p>
          <hr style="border: 0; border-top: 1px solid #f1f5f9; margin: 30px 0;" />
          <p style="color: #64748b; font-size: 12px; font-style: italic;">City Pulse Collective</p>
        </div>
      </div>
    ...
    await sendMsg91Email({
      to: email,
      subject: `Welcome to City Pulse — ${partnerType} Onboarding`,
      html
    });

    logger.info(`Partner onboarding email sent via MSG91 to ${email} (${partnerType})`);
  } catch (err) {
    logger.error('Failed to send partner onboarding email via MSG91', err);
  }
};
