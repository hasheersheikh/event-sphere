...
const Index = () => {
  ...
  const categories = [
    { label: "Music", icon: Music, gradient: "from-blue-500/15 to-indigo-500/15", border: "border-blue-500/20", text: "text-blue-600 dark:text-blue-400" },
    { label: "Arts", icon: Camera, gradient: "from-orange-500/15 to-rose-500/15", border: "border-orange-500/20", text: "text-orange-600 dark:text-orange-400" },
    { label: "Technology", icon: Cpu, gradient: "from-violet-500/15 to-purple-500/15", border: "border-violet-500/20", text: "text-violet-600 dark:text-violet-400" },
    { label: "Meetups", icon: Sparkles, gradient: "from-emerald-500/15 to-teal-500/15", border: "border-emerald-500/20", text: "text-emerald-600 dark:text-emerald-400" },
    { label: "Sports", icon: Zap, gradient: "from-red-500/15 to-orange-500/15", border: "border-red-500/20", text: "text-red-600 dark:text-red-400" },
    { label: "Business", icon: TrendingUp, gradient: "from-pink-500/15 to-rose-500/15", border: "border-pink-500/20", text: "text-pink-600 dark:text-pink-400" },
  ];
      ...
      <main className="flex-1">
        ...
        >
          ...
          <div className="relative z-10 w-full max-w-2xl mx-auto px-6 text-center">
            ...
            <motion.div
              {...fadeUp(0.05)}
              className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-primary/8 dark:bg-primary/10 border border-primary/20 text-primary text-[10px] font-black uppercase tracking-[0.5em] mb-5"
            >
            ...
            {/* title */}
            <h1 className="font-black tracking-tighter leading-[0.9] mb-5 text-[clamp(2.4rem,6vw,4.5rem)]">
              <WordReveal text="Find Your" baseDelay={0.1} />
              <br />
              <motion.span
                className="text-gradient italic font-light inline-block"
        ...
        >
          ...
          <div className="relative flex-1 overflow-hidden">
            <div className="h-full overflow-x-auto overflow-y-hidden px-6 md:px-10 py-4 scrollbar-hide">
              <div className="flex gap-3.5 h-full" style={{ width: "max-content" }}>
                {isLoading
          ...
          <div className="container">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
              {categories.map((cat, i) => (
                <Link key={i} to={`/events?category=${encodeURIComponent(cat.label)}`}>
                  <motion.div
                  ...
                  >
                    <div className={`p-2.5 rounded-lg bg-white/60 dark:bg-white/8 ${cat.text}`}>
        ...
        ════════════════════════════════════════════ */}
        <section className="py-16 border-t border-border/10 relative overflow-hidden" ref={featuredRef}>
          {/* ambient glow */}
          ...
          <div className="container relative z-10">
            ...
            {isLoading ? (
              ...
              <div className="grid grid-cols-1 md:grid-cols-12 gap-4">
                ...
                {/* right two cards */}
                {upcomingEvents.slice(1, 3).map((event: Event, i: number) => (
                  <motion.div
                    key={event._id}
        ...
        <section className="py-24 container">
          <motion.div
            ...
            className="relative overflow-hidden rounded-2xl border border-border/20 bg-gradient-to-br from-primary/5 via-background to-accent/5 px-8 md:px-16 py-16 md:py-20 shadow-xl"
          >
            {/* background blobs */}
            <Blob
              className="w-72 h-72 -top-20 -left-20 bg-primary/10 dark:bg-primary/8 blur-[90px]"
              animate={{ scale: [1, 1.2, 1], opacity: [0.5, 0.8, 0.5] }}
              duration={10}
            />
            <Blob
              className="w-64 h-64 -bottom-16 -right-16 bg-accent/10 dark:bg-accent/8 blur-[80px]"
            ...
            >
              <svg width="80" height="80" viewBox="0 0 100 100" fill="none">
                <path
                  d="M50 0 L58 35 L100 42 L65 58 L72 100 L50 75 L28 100 L35 58 L0 42 L42 35 Z"
            ...
            <div className="relative z-10 max-w-2xl">
              <motion.div
                ...
                viewport={{ once: true }}
                transition={{ delay: 0.1, duration: 0.5 }}
                className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-primary/8 border border-primary/20 text-primary text-[10px] font-black uppercase tracking-[0.5em] mb-6"
              >
              ...
              <motion.h2
                className="text-3xl md:text-5xl font-black tracking-tight leading-[0.9] mb-4"
                initial={{ opacity: 0, y: 16 }}
                whileInView={{ opacity: 1, y: 0 }}
                viewport={{ once: true }}
              ...
              >
                ...
                <span className="text-gradient italic font-light">next event?</span>
              </motion.h2>

              <motion.p
                className="text-muted-foreground text-sm md:text-base leading-relaxed mb-8 max-w-lg"
                initial={{ opacity: 0, y: 12 }}
                whileInView={{ opacity: 1, y: 0 }}
                viewport={{ once: true }}
                transition={{ delay: 0.3, duration: 0.5 }}
              >
                Join thousands of organizers on EventSphere — sell tickets, manage
                attendees, and grow your audience.
              </motion.p>
              ...
              >
                <Link to="/auth">
                  <Button className="h-12 px-8 rounded-xl font-bold uppercase tracking-widest text-xs bg-primary text-primary-foreground shadow-md hover:shadow-lg hover:shadow-primary/25 hover:scale-[1.02] active:scale-95 transition-all duration-300 group">
                    Create Account
                ...
                <Link to="/events">
                  <Button
                    variant="outline"
                    className="h-12 px-8 rounded-xl font-bold uppercase tracking-widest text-xs border-border/50 hover:border-primary/40 hover:text-primary transition-all duration-300"
                  >
                    Browse Events
                  </Button>
                </Link>
